...
for sm in surahmeta:
    osm = SurahMeta(
        surah_number=sm.get("number"),
        name_string=sm.get("name"),
        surah_ayat_max=sm.get("numberOfAyahs"),
    )
    osm.save()
...
for i in range(15):
    ...
    lenayat = len(
        QuranMeta.objects.get(surah_number=sn, ayat_number=an).ayat_string.split(" ")
    )
